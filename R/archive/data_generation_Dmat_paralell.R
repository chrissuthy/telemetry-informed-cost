...
ref.Dmat_i <- function(from, to, r, dist_mat){
  #browser()
  
  from <- as.data.frame(matrix(from, ncol=2))
  colnames(from) <- c("x","y")
  to   <- as.data.frame(matrix(to, ncol=2))
  colnames(to) <- c("x","y")
  
  from_cell <- as.numeric(raster::extract(x = r, y = from, cellnumbers=T)[,1])
  to_cell   <- as.numeric(raster::extract(x = r, y = to,   cellnumbers=T)[,1])
  result    <- matrix(dist_mat[from_cell, to_cell], nrow = nrow(from))
  return(result)
}
...
ss <- for_ss %>%
  #aggregate(., fact = 4) %>% THIS IS NO GOOD!! NOT USED, DIFFFERENT RES, AND MESSES UP TRAPS etc!!
  as.data.frame(., xy=T) %>%
  select(x, y) %>%
...
for(sim in 1:sims){
  ...
  print("Landscape gen time")
  t2 = Sys.time()
  print(t2-t1)
  
  #----Landscape Dmat----
  
  # Slice up landscape into 10 pieces
  d_ix <- data.frame(
    slice1 = floor(seq(1, nrow(landscape), length.out = 11))[1:10],
    slice2 = floor(seq(1, nrow(landscape), length.out = 11))[2:11])
  d_ix[2:10, 1] <- d_ix[2:10, 1]+1
  
  # Make distance matrix for each piece
  dmat <- list()
  dmat <- foreach(j=1:10, .packages = c(.packages())) %dopar% {
    m <- costDistance(
      tr1Corr,  
      from = landscape %>% select(x,y) %>% slice(d_ix[j,1]:d_ix[j,2]) %>% as.matrix(),
  ...
  tracks <- foreach(j=1:N, .packages = c(.packages())) %dopar% {
    ...
    # All coordinates of local_ss_r
    dmat_from <- dmat_to <- local_ss %>% select(x,y) %>% as.matrix()

    # Create Indvidual D matrix (from = rows, to = cols)
    Dmat_i <- ref.Dmat_i(
      from = dmat_from,
      to = dmat_to,
      r = landscape_r, # THIS NEEDS TO BE FULL LANDSCAPE
      dist_mat = dmat
    )
    ...
    for(i in 2:nfix){
      ...
      # Distance from last position to each pixel in local ss (subset above)
      D <- ref.Dmat_i(
        from = as.matrix(d_from),
        to = as.matrix(d_to),
        r = landscape_r,
        dist_mat = dmat
      )
