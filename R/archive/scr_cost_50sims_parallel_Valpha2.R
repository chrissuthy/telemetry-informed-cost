...
colnames(out) <- c("alpha2", "sigma", "p0", "d0")
...
results <- foreach(sim=1:nsims, .packages = c(.packages())) %dopar% {
  ...
  file <- paste0("C:/Users/dupon/Desktop/cost_sims_10032020/sim", sim, ".txt")
...
out_test %>%
  ...
  theme(legend.position = "none") + ylim(-12, 7)
...
out_test %>%
  ...
  theme(legend.position = "none") + ylim(-74, 100)
...
# Estimate convergence ove nsims
...
test <- out %>%
  ...
  group_by(true) %>%
  mutate(Sims = 1:n(),
         CM_alpha2 = cummean(alpha2) - true,
         CM_density = cummean(d0) - (300/6400)) 

p1 <- ggplot(data = test, aes(x = Sims, y = CM_alpha2, group = true, color = true)) +
  geom_line(size = 1) +
  geom_hline(yintercept = 0, color = "red", size = 1, lty = 2) +
  theme_minimal() + labs(x = "Number of simulations", y = "Cummulative mean", title = "Alpha2", color = "Alpha2")

p2 <- ggplot(data = test, aes(x = Sims, y = CM_density, group = true, color = true)) +
  geom_line(size = 1) +
  geom_hline(yintercept = 0, color = "red", size = 1, lty = 2) +
  theme_minimal() + #ylim(NA, 0.025) +
  labs(x = "Number of simulations", y = "Cummulative mean", title = "Density", color = "Alpha2")
