...
out_it <- matrix(NA, nrow = nsims, ncol = 6)
...
rb <- data.frame(
  alpha2 = 100*((out_it[,"alpha2"]-alpha2)/alpha2),
  upsilon = 100*((out_it[,"upsilon"]-upsilon)/upsilon),
  psi = 100*((out_it[,"psi"]-psi)/psi),
  sigma = 100*((out_it[,"sigma"]-sigma)/sigma),
  p0 = 100*((out_it[,"p0"]-p0)/p0),
  d0 = 100*((out_it[,"d0"]-d0)/d0)
)
