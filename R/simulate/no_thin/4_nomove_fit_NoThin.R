...
# Y array
file      <- paste0("output/", select_ups, "/model_data/y_NoThin.RData")
y         <- readRDS(file)
...
results <- foreach(sim=1:sims, .packages = c(.packages())) %dopar% {
  ...
  file <- paste0("output/", select_ups, "/", file_id, "/screco_", sim, "_NoThin.txt")
...
# SAVE IT ALL
file <- paste0("output/", select_ups, "/", file_id, "/results_NoThin.RData")
saveRDS(results, file)
file <- paste0("output/", select_ups, "/", file_id, "/results_NoThin.txt")
