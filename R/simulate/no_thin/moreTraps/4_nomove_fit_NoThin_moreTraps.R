...
results <- foreach(sim=1:sims, .packages = c(.packages())) %dopar% {
  ...
  file <- paste0("output/", select_ups, "/", file_id, "/screco_", sim, "_NoThin_moreTraps.txt")
...
# SAVE IT ALL
file <- paste0("output/", select_ups, "/", file_id, "/results_NoThin_moreTraps.RData")
saveRDS(results, file)
file <- paste0("output/", select_ups, "/", file_id, "/results_NoThin_moreTraps.txt")
