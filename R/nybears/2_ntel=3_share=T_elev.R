...
spatdata_base <- as.data.frame(elev, xy = T)[,1:2]
...
colnames(out) <- c("model", "cost", "sigma", "psi", "sig_det", "p0", "d0")
out
write.table(out, "./output/nybears/ntel=3_share=T_MSE_SE_elev.txt")

save.image("./output/nybears/ntel=3_share=T_elev.RData")
