...
out <- rbind(final, ASE) %>% as.data.frame() %>% mutate(model = "~forest") %>% select(model, everything())
...
write.table(out, "./output/nybears/ntel=3_share=F_MSE_SE_forest.txt")
save.image("./output/nybears/ntel=3_share=F_forest.RData")
